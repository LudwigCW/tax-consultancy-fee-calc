...
export const translations: Record<Language, TranslationKeys> = {
  ...
  zh: {
    meta: {
      title: 'StBVV 费用计算器',
      description:
        '根据德国《税务顾问报酬条例》(StBVV 2025) 在线计算税务顾问费用——年度报表、增值税、企业所得税和营业税。',
    },
    language: {
      de: 'Deutsch',
      zh: '中文',
    ...
    hero: {
      ...
      title: '计算税务顾问费用',
      text: '依据《税务顾问报酬条例》(StBVV) 计算年度报表、增值税申报、企业所得税申报和营业税申报的费用。',
      loadExample: '加载示例',
    },
    sections: {
      companyData: '企业数据',
    ...
    inputs: {
      ...
      bilanzsumme: {
        label: '资产负债表总额',
        hint: '年度报表标的额：资产负债表总额与营业额的平均值（最低 30.000 €）',
      },
      gewerbeertrag: {
        label: '营业收益',
    ...
    services: {
      jahresabschluss: '年度报表（资产负债表/损益表）',
      umsatzsteuer: '增值税申报',
    ...
    steps: [
      {
        title: '确定标的额',
        text: '年度报表：资产负债表总额与营业额的平均值（最低 30.000 €）。增值税：营业额的 10%。企业所得税和营业税：分别为应税收入和营业收益（各最低 8.000 €）。',
      },
      {
        title: '查表确定全额费用',
        text: '税务申报适用 A 表，年度报表适用 B 表（StBVV 2025）。若标的额介于两档之间，适用较高一档（§ 13 StBVV）。',
      },
      {
        title: '乘以十分率',
